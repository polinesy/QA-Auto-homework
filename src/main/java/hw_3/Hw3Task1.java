...
public static void alertPage (WebDriver driver){
    ...
    for (String tab : tabs) {
        driver.switchTo().window(tab);
        if (driver.getCurrentUrl().startsWith("https://www.hyrtutorials.com/p/alertsdemo.html")) {
            try {
                driver.findElement(By.id("ez-accept-all")).click();
            } catch (NoSuchElementException e) {
                System.out.println("No cookies button");
    ...
    alert2.dismiss();
    WebElement text2 = driver.findElement(By.id("output"));
    ...
    alert3.accept();
    WebElement text3 = driver.findElement(By.id("output"));
